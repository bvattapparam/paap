...
get_carothers_data();
 ...
 function get_carothers_data() {
  ...
  $qry_count = mysql_query("SELECT * FROM VIEW_CAR_OTHERS WHERE CAR = '" .$carmodel."'");
  $num_rows = mysql_num_rows($qry_count);

  // QUERY TO GET SUM
  $qry_sum = mysql_query("SELECT SUM(AMOUNT) as TOTAL_AMOUNT FROM VIEW_CAR_OTHERS  WHERE CAR = '" .$carmodel."'");
  $sum_rows = mysql_fetch_assoc($qry_sum); 
  $sum = $sum_rows['TOTAL_AMOUNT'];

  // QUERY TO GET THE LIST OF ITEMS
  $qry = "SELECT * FROM VIEW_CAR_OTHERS WHERE CAR = '" .$carmodel."' LIMIT $limit OFFSET $offset" ;
